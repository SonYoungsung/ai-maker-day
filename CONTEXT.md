...
중·고등학생 대상 **원데이 AI 클래스(4시간)** 웹사이트. 두 역할: **(1) 강의 자료(스킬 3종) 배포**, **(2) 학생 결과물(HTML 보고서) 수집 + 실시간 대시보드**. GitHub Pages 배포 예정.
...
- **[결정 2026-09-22 · 현장 피드백] draft-first 로 전면 전환, 5종 → 3종.** 시범 운영 피드백이 **"질문이 너무 많다. 빠르게 결과물을 보고 그걸 바탕으로 원하는 부분을 깎아나가는 게 낫다"** 로 모였다. 기존 설계(질문 → 결과)가 정반대 방향이라 원칙부터 뒤집었다.
  - **새 흐름**: 씨앗 질문 1개 → 아이디어 3개 → 고르기 → **작동하는 MVP 즉시 생성** → 학생이 열어서 확인 → **깎기 루프 반복** → 발표.
  - **`02-blueprint`(설계) 단계 폐기.** 화면·흐름·분위기를 문서로 먼저 정하지 않는다. AI가 MVP를 만들며 임의로 정하고, 보고서의 **"AI가 정한 것"** 목록으로 드러내면 학생이 그걸 보고 뒤집는다. 결정권은 그대로 학생에게 있되, 빈 종이가 아니라 돌아가는 물건을 보면서 결정한다.
  - **새 단계**: `01-idea-mvp`(🚀 아이디어→첫 버전) → `02-upgrade`(🔨 깎기, **반복 단계**) → `03-demo`(🎤 발표 준비).
  - **`02-upgrade` 는 반복 제출된다.** 한 바퀴 = 한 제출. `Dashboard.tsx` 의 `versionKey`(학생|단계|날짜)가 이미 버전을 묶으므로, 같은 단계를 여러 번 내면 **프로젝트가 깎여가는 과정이 버전 히스토리로 쌓인다.** 단계 통과보다 강사에게 유용한 신호다. build-skills 의 STAGES 에 `handoff` 오버라이드를 둬서 다음 단계로 밀지 않고 "또 돌래 / 여기까지 할래"를 학생이 고르게 한다.
  - **preamble 전면 교체.** 기존 "결과 직행 금지 / 질문 한 번에 하나씩 / 자동 완성하면 배움이 사라진다"를 **씨앗질문 최소 → 즉시 초안 → "내가 이렇게 정했어" 공개 → 선택지로 깎기 → 바꾸는 건 한 번에 하나**로 교체. 이걸 안 바꾸면 새 스킬이 작동하지 않는다.
  - **`tests/rubric.md` 반전.** 옛 A2("직행 = 실패")가 새 설계에선 의도된 동작이라 그대로 두면 회귀 테스트가 새 스킬을 전부 불합격시킨다. 새 실패 조건: 씨앗 질문 3개 이상 / 초안 없이 질문만 3턴 / "AI가 정한 것" 미공개 / 열린 질문만 하고 선택지 미제공 / 학생이 고르지 않은 것 변경 / 반복 단계를 일방적으로 다음으로 밀기.
  - **시간 표기 삭제.** 순서만 지키고 속도는 학생마다 유동적으로 간다. `SkillMeta.period` 필드 제거.
...
**완료 (2026-09-22 — draft-first 3단계 전환):**
- `report-templates/interaction-preamble.md` **전면 재작성** — draft-first 8개 규칙.
- `skills/` 3종 재작성: `01-idea-mvp`(씨앗질문 1개→아이디어 3개→MVP 즉시 생성→"AI가 정한 것"→첫 깎기) / `02-upgrade`(제안 3개 먼저→학생이 고름→한 군데만→"또 돌래?" 반복) / `03-demo`(질문 0개, 앞 기록으로 발표문 초안→말투 깎기). 옛 5개 trash.
- `scripts/build-skills.mjs` STAGES 3종 + `handoff` 오버라이드 지원(반복 단계용).
- `site/src/lib/skills.ts` 3종으로 교체. `npm run build` 통과, 강의자료 모달 렌더 확인.
- `report-templates/examples/` 3종 재작성, `verify-examples.mjs` 3/3 통과.
- `tests/rubric.md` draft-first 기준으로 반전(A~E 16항목).

**완료 (2026-09-22 — 4시간 기준 5단계 통합, 위 전환으로 대체됨):**
